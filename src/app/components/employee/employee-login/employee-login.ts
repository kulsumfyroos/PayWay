...
export class EmployeeLogin implements AfterViewInit, OnDestroy {
  ...
  ngAfterViewInit(): void {
    ...
    // Real-time validation for employeeId
    ...
    if (employeeIdEl) {
      ...
      employeeIdEl.addEventListener('input', h);
      this.realTimeHandlers.push({ elId: 'employeeId', handler: h });
    }
  ...
  private onSubmit(e: Event): void {
    ...
    const employeeId = (document.getElementById('employeeId') as HTMLInputElement | null)?.value.trim() ?? '';
    const password = (document.getElementById('password') as HTMLInputElement | null)?.value ?? '';

    this.clearErrors();

    // Validate Employee ID
    if (employeeId.length < 7 || isNaN(Number(employeeId))) {
      this.showError('employeeId', 'Employee ID must be 7 digits');
      return;
    }
